...
import com.common.lib.log.LogFileUtils;
...
//        Log.i("localData",new Gson().toJson(ProtocalFactory.createPLoginInfo(loginInfo)));
    ...
    private int send(byte[] fullProtocalBytes, int dataLen) {
        ...
        if (ds != null && ds.isActive()) {// && [ClientCoreSDK sharedInstance].connectedToServer)
            //发送用户信息 默认都是ture 根据 handleProtocalImpl 这个方法返回的状态来回调对应的事件回调 比如 登录成功 50 断开连接 51
